...
function createXmasTree(height,character) {
    let filesHeightTree = []
    let lengthFileTree = [character]
    let lengthCharacter = character.length
    for (let index = 0; index < height; index++) {
        while(lengthCharacter % 2){
            
            lengthCharacter = lengthCharacter + 1
            lengthFileTree.push(character)
            
        }
        lengthCharacter = lengthCharacter + 1
        lengthFileTree.push(character)
        filesHeightTree.push(lengthFileTree)
        
        
    }
   console.log(lengthFileTree,filesHeightTree);
